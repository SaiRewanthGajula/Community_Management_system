...
import { AlertCircle, Car, Calendar, Users, DollarSign, MessageSquare } from 'lucide-react';
...
interface Notification {
  ...
  description: string;
  timestamp: number;
  type: 'visitors' | 'bookings' | 'vehicles' | 'billing' | 'complaints';
}
...
const AdminDashboard: React.FC = () => {
  ...
  const [resolveDescription, setResolveDescription] = useState<string>('');
  const [activeSection, setActiveSection] = useState<'visitors' | 'bookings' | 'vehicles' | 'billing' | 'complaints' | null>(null);
  ...
  useEffect(() => {
    ...
    console.log(`AdminDashboard rendered ${renderCount.current} times`);
  });
  ...
  useEffect(() => {
    ...
    socketRef.current.on('newNotification', (notification: any) => {
      if (notification.type === 'bill_reminder' || notification.type === 'bill_overdue') {
        ...
        setNotifications((prev) => [
          {
            id: Date.now(),
            title: `Bill ${notification.type === 'bill_reminder' ? 'Reminder' : 'Overdue'}`,
            description: `Bill for ${notification.user_name || 'User'} (${notification.amount}) is ${notification.type === 'bill_reminder' ? 'due' : 'overdue'} at ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`,
            timestamp: Date.now(),
            type: 'billing',
          },
          ...prev.slice(0, 9),
    ...
    socketRef.current.on('complaintUpdated', (data: any) => {
      fetchComplaints();
      setNotifications((prev) => [
        {
          id: Date.now(),
          title: `Complaint ${data.status === 'resolved' ? 'Resolved' : 'Updated'}`,
          description: `Complaint "${data.title}" from ${data.unit || 'N/A'} marked as ${data.status} at ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`,
          timestamp: Date.now(),
          type: 'complaints',
        },
        ...prev.slice(0, 9),
  ...
  const fetchBills = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.status === 403
        ? 'Access denied to bills endpoint.'
        : err.response?.status === 404
        ? 'Bills endpoint not found. Please check the server configuration.'
        : err.response?.data?.error || 'Failed to load bills';
      setError(errorMessage);
  ...
  const fetchComplaints = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem('societyToken');
      if (!token) throw new Error('No authentication token');
      const response = await axios.get(`${apiBase}/complaints`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setComplaints(Array.isArray(response.data) ? response.data : []);
    } catch (err: any) {
      const errorMessage = err.response?.status === 403
        ? 'Access denied to complaints endpoint.'
        : err.response?.status === 404
        ? 'Complaints endpoint not found. Please check the server configuration.'
        : err.response?.data?.error || 'Failed to load complaints';
      setError(errorMessage);
  ...
  const handlePinVerification = async (visitorId: number) => {
    ...
    try {
      ...
      setPinError('');
      fetchVisitors();
      socketRef.current?.emit('visitorUpdated', { 
        id: visitorId, 
        name: visitors.find(v => v.id === visitorId)?.name, 
        unit: visitors.find(v => v.id === visitorId)?.unit, 
  ...
  const handlePayNow = async (billId: number) => {
    try {
      ...
      socketRef.current?.emit('newNotification', {
        type: 'bill_paid',
        user_name: bills.find(b => b.id === billId)?.user_name,
        amount: bills.find(b => b.id === billId)?.amount,
      });
    } catch (err: any) {
      const errorMessage = err.response?.status === 403
        ? 'Access denied to pay bill.'
        : err.response?.data?.error || 'Failed to pay bill';
      setError(errorMessage);
  ...
  const handleResolveComplaint = async (complaintId: number) => {
    ...
    try {
      ...
      if (!token) throw new Error('No authentication token');
      await axios.put(
        `${apiBase}/complaints/${complaintId}/resolve`,
        { resolution_description: resolveDescription },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      fetchComplaints();
      socketRef.current?.emit('complaintUpdated', {
        id: complaintId,
        title: complaints.find(c => c.id === complaintId)?.title,
        status: 'resolved',
        unit: complaints.find(c => c.id === complaintId)?.unit,
      });
      setResolveDescription('');
    } catch (err: any) {
      const errorMessage = err.response?.status === 403
        ? 'Access denied to resolve complaint.'
        : err.response?.data?.error || 'Failed to resolve complaint';
      setError(errorMessage);
  ...
  useEffect(() => {
    ...
    fetchBills();
    fetchComplaints();
  }, [user, isAdmin, fetchVisitors, fetchPendingBookings, fetchVehicleLogs, fetchBills, fetchComplaints]);
  ...
  const stats = useMemo(
    () => [
      ...
      {
        label: 'Pending Bills',
        value: bills.filter(b => b.status === 'pending' || b.status === 'upcoming').length.toString(),
        icon: DollarSign,
        color: 'text-green-500',
      },
      {
        label: 'Open Complaints',
        value: complaints.filter(c => c.status === 'open' || c.status === 'in-progress').length.toString(),
        icon: MessageSquare,
        color: 'text-red-500',
      },
    ],
    [visitors, pendingBookings, vehicleLogs, bills, complaints]
  );
  ...
  if (!isAdmin) {
    return <Card className="p-4 mx-auto max-w-md sm:max-w-lg">Access denied. Admin role required.</Card>;
  }
      ...
      {loading ? (
        ...
        <>
            ...
            {stats.map((stat) => (
              <Card
                ...
                onClick={() => setActiveSection(
                  stat.label.toLowerCase().includes('visitors') ? 'visitors' :
                  stat.label.toLowerCase().includes('bookings') ? 'bookings' :
                  stat.label.toLowerCase().includes('vehicles') ? 'vehicles' :
                  stat.label.toLowerCase().includes('bills') ? 'billing' :
                  'complaints'
                )}
              >
              ...
              notifications.map((notification) => (
                ...
                >
                  <div className="flex-shrink-0 mr-4">
                    ...
                    {notification.type === 'billing' && <DollarSign className="w-6 h-6 text-green-500" />}
                    {notification.type === 'complaints' && <MessageSquare className="w-6 h-6 text-red-500" />}
                  </div>
          ...
          {activeSection && (
            <Card
              title={
                activeSection === 'visitors' ? 'Pending Visitors' :
                activeSection === 'bookings' ? 'Pending Amenity Bookings' :
                activeSection === 'vehicles' ? 'Vehicle Logs' :
                activeSection === 'billing' ? 'Current Bills' :
                'Open Complaints'
              }
              icon={
                activeSection === 'visitors' ? <Users className="w-5 h-5" /> :
                activeSection === 'bookings' ? <Calendar className="w-5 h-5" /> :
                activeSection === 'vehicles' ? <Car className="w-5 h-5" /> :
                activeSection === 'billing' ? <DollarSign className="w-5 h-5" /> :
            ...
            >
              ...
              {activeSection === 'billing' && (
                bills.filter(b => b.status === 'pending' || b.status === 'upcoming').length === 0 ? (
                  <p className="text-gray-600">No current bills.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Description</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">User</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Amount</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6 hidden sm:table-cell">Due Date</th>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {bills
                          ...
                          .map((bill) => (
                            <tr key={bill.id} className="block sm:table-row">
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Description:'] before:font-medium sm:before:content-none">
                                {bill.description}
                              </td>
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['User:'] before:font-medium sm:before:content-none">
                                {bill.user_name ? `${bill.user_name} (${bill.user_unit || 'N/A'})` : bill.user_id}
                              </td>
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Amount:'] before:font-medium sm:before:content-none">
                                ₹{Number(bill.amount).toFixed(2)}
                              </td>
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Due_Date:'] before:font-medium sm:before:content-none hidden sm:table-cell">
                                {formatDateTimeToIST(bill.due_date)}
                              </td>
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Status:'] before:font-medium sm:before:content-none">
                                {bill.status}
                              </td>
                            </tr>
                          ))}
              ...
              {activeSection === 'complaints' && (
                complaints.filter(c => c.status === 'open' || c.status === 'in-progress').length === 0 ? (
                  <p className="text-gray-600">No open complaints.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Title</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Description</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Unit</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6 hidden sm:table-cell">Date</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Status</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Priority</th>
                          <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sm:px-6">Action</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {complaints
                          ...
                          .map((complaint) => (
                            <tr key={complaint.id} className="block sm:table-row">
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Title:'] before:font-medium sm:before:content-none">
                                {complaint.title}
                              </td>
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Description:'] before:font-medium sm:before:content-none">
                              ...
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Date:'] before:font-medium sm:before:content-none hidden sm:table-cell">
                              ...
                              <td className="block px-4 py-2 sm:table-cell sm:px-6 sm:py-4 before:content-['Priority:'] before:font-medium sm:before:content-none">
